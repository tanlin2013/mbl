...
from tests.workflow.mlflow.helper_functions import LOCALHOST, get_safe_port
...
# NB: We explicitly wait and timeout on server shutdown in order to ensure that pytest
# output reveals the cause in the event of a test hang due to the subprocess not
# exiting.
...
def _init_server(backend_uri, root_artifact_uri):
    """
    Launch a new REST server using the tracking store specified by backend_uri and root
    artifact directory specified by root_artifact_uri.
    :returns A tuple (url, process) containing the string URL of the server and a handle
             to the server process (a multiprocessing.Process object).
    """
    ...
    _logger.info(
        f"Launching tracking server against backend URI {backend_uri}. "
        f"Server URL: {url}"
    )
    return url, process
