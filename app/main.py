...
if __name__ == "__main__":
    ...
    chi = (
        st.sidebar.radio("Truncation dim chi", options_chi)
        if table == "tsdrg"
        else None
    )
    relative_offset = (
        st.sidebar.radio("Relative offset", (0.1, 0.5, 0.9))
        if table == "tsdrg"
        else None
    )
    ...
    if n_conf is not None:
        ...
        mini_df = pd.concat(
            [grouped.get_group(g) for g in list(grouped.groups)[:n_conf]]
        )
    else:
        mini_df = df.copy()
